...
public abstract class ComplexQbject extends Qbject {
	...
	public ComplexQbject(Matheors p, float massKg, float strength, Coordinates compos, Vector initVelocity) {
	...
	public boolean determineIfCollisionOccurredWith(Qbject other) {
		if (other instanceof ComplexQbject) {
			...
			for (Coordinates ct : this.vertices) {
				...
				for (Coordinates co : cother.vertices) {
			...
			return false;
		} else { // if (other instanceof SimpleQbject) {
			SimpleQbject sother = (SimpleQbject) other;
			for (Coordinates v : this.vertices) {
				if (calcDistance(v, sother.compos) < sother.radious)
					return true;
			}
			return false;			
		}
